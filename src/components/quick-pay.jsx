  ...
  return (
    <div id="quick-pay">
        ...
        {quickCheckout.map((item, index) => {
          return (
            <PayForm
              ...
              data={item}
              page={
                quickCheckout.length > 1
                  ? `${index + 1} of ${quickCheckout.length}`
                  : ""
              }
            />
          );
      ...
      <style jsx>{`
        ...
        #quick-pay .form-box::-webkit-scrollbar {
          display: none;
        }
      `}</style>
    </div>
    ...
    <form onSubmit={handleSubmit} className="pay-form">
      {product !== null && (
        <div className="product-details">
          <div className="timer">
            {data.expired ? "Extra" : ""} {timeLeft}s Left
          </div>
          ...
          <div className="details">
            <h2 className="price">
              Gh&cent; {parseFloat(product.price).toFixed(2)}
            </h2>
            <input
              required
              value={formData.name}
            ...
            {success ? (
              <div style={{ background: "#fff" }} className="overlay">
