...
function ProductsProvider({ children }) {
  ...
  const {
    ...
    notifyCancelRequestCheckout,
    notifyDeniedRequest,
    notifyUnheld,
    notifyRequestCheckoutSuccess,
  } = useNotification();
  ...
  const increaseWatch = (product) => {
    productsRef
      .doc(product.id)
      .update({ watchCount: product.watchCount + 1 })
      .catch((error) => {
  ...
  const joinHolders = (userId, product) =>
    productsRef
      .doc(product.id)
      .update({ holders: [...product.holders, userId] });

  const unlockProduct = (id) => productsRef.doc(id).update({ locked: false });

  const getRequestById = (reqId) => {
    return purchaseReqRef
      .doc(reqId)
      .get()
      .then((res) => res.data());
  };
  ...
  const cancelRequestPurchase = (userId, product, callback = () => {}) => {
    if (product && userId) {
      purchaseReqRef
        ...
        .then((res) => {
          callback();
          res.docs.length && purchaseReqRef.doc(res.docs[0].data().id).delete();
        })
  ...
  const unholdProduct = (userId, product) => {
    if (userId && product) {
      ...
      productsRef
        .doc(product.id)
        .update({ heldBy: "", status: 0, purchaseRequests: [] })
        .then((_) => {
          // do something here
  ...
  const addToWishList = (userId, product) => {
    ...
    if (product.wishlist.includes(userId)) {
      update = {
        wishlist: [...product.wishlist.filter((id) => id !== userId)],
      };
    } else {
  ...
  const comment = (by, message, product) => {
    productsRef
      .doc(product.id)
      .update({ comments: [{ by, comment: message }, ...product.comments] })
      .catch(({ message }) => {
        createError(message);
      });
  };
